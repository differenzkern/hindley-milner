...
impl<'a> EvalContext<'a> {
        ...
        match tm {
            ...
            Expr::Match(aref, expr, arms) => {
                if let Sem::ConsApp(cons, args) = self.meaning(expr, ctx).as_ref() {
                    ...
                    Rc::new(Sem::Syn(Rc::new(Expr::Match(*aref, expr.clone(), arms.clone()))))
                }
            }
    ...
    fn reify(&self, sm: Rc<Sem>) -> Option<Rc<Expr>> {
        match sm.as_ref() {
            Sem::Lam(_, tm) => {
               None
            },
            Sem::ConsApp(cref, args) => {
                let args = args.iter().map(|arg| self.reify(arg.clone()).unwrap()).collect();

                Some(Rc::new(Expr::ConsApp(*cref, args)))
            }
            Sem::Syn(expr) => None,
        }
    }
